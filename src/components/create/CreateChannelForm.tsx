...
import { SEPOLIA_ADDRESSES } from '@/src/lib/addresses';
...
const CreateChannelForm: React.FC = () => {
  ...
  const admin1Cleaned = isAddress(formData.admin1) ? formData.admin1 : zeroAddress
  const admin2Cleaned = isAddress(formData.admin2) ? formData.admin2 : zeroAddress
  const admin3Cleaned = isAddress(formData.admin3) ? formData.admin3 : zeroAddress
  const curationPassCleaned = isAddress(formData.curationPass) ? formData.curationPass : zeroAddress
  ...
  const createAccessInitArray = (arrayOfAdmins: any) => {
    ...
    const accessModuleInit = encodeAbiParameters(
        parseAbiParameters('address, (address, uint8)[]'),
        // @ts-ignore
        [curationPassCleaned, createAccessInitArray([admin1Cleaned, admin2Cleaned, admin3Cleaned])]
    )  

    const logicInitInput = encodeAbiParameters(
        parseAbiParameters('bool, address, bytes'),    
        [
            false, // initial pause state == not paused
            // @ts-ignore
            SEPOLIA_ADDRESSES.ERC721.HYBRID_ACCESS, // access module == hybrid access mdule
  ...
  return (
    <form onSubmit={handleSubmit} className="text-black w-full max-w-xl mx-auto grid grid-cols-2 gap-4">
    <div className="col-span-2">
      ...
      <input
        ...
        className="text-black mt-1 block w-full border border-gray-300 rounded py-2 px-3 text-sm focus:ring-indigo-500 focus:border-indigo-500"
        required
      />
    </div>

    <div className="col-span-2">
      <label htmlFor="admin1" className="block text-sm font-medium text-gray-400">
        Admin 1:
      </label>
      <input
        type="text"
    ...
    <div className="col-span-1">
      <label htmlFor="admin2" className="block text-sm font-medium text-gray-400">
        Admin 2 (optional):
      </label>
      <input
        type="text"
    ...
    <div className="col-span-1">
      <label htmlFor="admin3" className="block text-sm font-medium text-gray-400">
        Admin 3 (optional):
      </label>
      <input
        type="text"
    ...
    <div className="col-span-2">
      <label htmlFor="curationPass" className="block text-sm font-medium text-gray-400">
